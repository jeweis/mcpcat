...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, public_paths: List[str] = None):
        ...
        self.public_paths = public_paths or [
            ...
            r"^/static/.*",  # 静态文件
            r"^/api/health$",  # 健康检查
            r"^/api/skills/bootstrap$",  # CLI 连接发现（不含凭证）
    ...
    def _check_server_auth_config(self, server_name: str) -> bool:
        ...
        """
        try:
            server_config = self.config_service.get_server_config(server_name) or {}
    ...
    def _is_catalog_server(self, server_name: str) -> bool:
        """判断是否为 Catalog 端点"""
        try:
            catalog = self.config_service.get_setting_section("catalog", {})
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            # 记录成功的认证
            logger.debug(
                f"认证成功: {key_config.name} ({key_config.permission.value}) -> {method} {path}"
            )
            ...
            return JSONResponse(
                status_code=500, content={"detail": "Authentication error"}
            )
