...
├── models/                 # 数据模型和模式
│   ├── __init__.py
│   └── mcp_config.py       # MCP 服务器配置模型、安全配置模型
└── services/               # 业务逻辑服务
    ├── __init__.py
    ├── config_service.py   # 配置管理
    ├── mcp_factory.py      # MCP 服务器工厂
    ├── security_service.py # API Key 和权限管理
...
- **MCPProxyApp**: ASGI 代理应用，支持动态服务器切换
- **ConfigService**: 处理配置加载和验证
- **MCPServerFactory**: 根据类型创建 MCP 服务器实例
- **SecurityService**: API Key 验证和权限管理
- **AuthMiddleware**: 请求认证和权限检查
- **Health & Server & Auth APIs**: 用于监控、管理和认证的 RESTful 端点
